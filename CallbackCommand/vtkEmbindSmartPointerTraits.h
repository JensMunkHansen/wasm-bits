...
template<typename T> struct smart_ptr_trait<vtkSmartPointer<T>>
{
    ...
    static pointer_type share(const pointer_type &r, T *ptr) { return pointer_type(ptr); }
...
template<typename T> vtkSmartPointer<T> MakeVTKSmartPtr() { return vtkSmartPointer<T>::New(); }
}// namespace vtk
